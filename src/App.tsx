...
//We will create a conditional Display , The button Start Quiz only show whtn Game over is set to true because Quiz is Completed or user completed all the questions
 ...
 //Asynchronous function
  ...
  //We can show the next question, if all 10 Questions done do GameOver, and value of the question which should be displayed
  ...
  const checkAnswer = (e: React.MouseEvent<HTMLButtonElement> ) => 
  {
    if (!gameOver) 
    {
      ...
      // We will uset AnswerObject to update the userAnswersState
      const answerObject = {  
      ...
      //We will use this object to update user answer state, setuseranswer use the prev state, spreaded out to new data, call the prev[...prev] and add the answer
      setUserAnswers(prev => [...prev, answerObject])
  }      //And this will be an Arrow Function  
  //We check if game is not over and user clicked on a button is correct or not
  };
  return (
    <>
    <GlobalStyle />
